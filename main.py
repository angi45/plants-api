...
async def get_plants():
    ...
    for index, plant in enumerate(plants_data, start=1):
        ...
        watering_text = plant_copy.get("Watering", "")
        plant_copy["watering_schedule"] = smart_watering_schedule(watering_text)
...
async def get_plant_by_local_id(local_id: int):
    ...
    if 1 <= local_id <= len(plants_data):
        ...
        watering_text = plant_copy.get("Watering", "")
        plant_copy["watering_schedule_days"] = smart_watering_schedule(watering_text)
